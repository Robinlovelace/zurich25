...
rand_route_sf <- bind_rows(
  net_dodgr_sf |> 
  semi_join(rand_route_edges,
   by = c("from_id", "to_id")),
net_dodgr_sf |> 
  semi_join(rand_route_edges,
   by = c("from_id"="to_id", "to_id"="from_id"))) # this is done as the network is undirected

tm_shape(net_dodgr_sf)+
  tm_lines("grey",lwd = 0.8)+
  tm_shape(od_rand)+
  tm_dots("firebrick", size = 0.5) +
  tm_shape(rand_route_sf) +
  tm_lines("steelblue", lwd = 2)

```

Using the same graph, we can perform All-or-Nothing assignment as follows:
```{r}
zone_centroids = st_centroid(zones)

# Find nearest vertices
zone_centroids$node <- net_dodgr_sf$from_id[st_nearest_feature(
  zone_centroids,
  net_dodgr_sf
)]
...
od_cpp <- od_calculated |>
  st_drop_geometry() |>
  select(O, D, interaction) |>
  left_join(
    zone_centroids |>
      st_drop_geometry() |>
      select(label, node),
...
flows_sf <- bind_rows(
  ...
  net_dodgr_sf |>
    ...
    inner_join(
      aon_flows |>
        select(-cost),
      by = c("from_id" = "to", "to_id" = "from")
    )
) |>
  summarise(flow = sum(flow), .by = edge_id)
...
```{r}
#| label: fig-aon-flows
#| fig-cap: "Estimated flows from All-or-Nothing assignment and cppRouting"
#| fig-dpi: 300
#| fig-width: 6
#| fig-height: 6
...
net_dodgr_sf |>
  inner_join(flows_sf,
   by = "edge_id") |>
    arrange(-flow) |> 
  tm_shape() +
tm_lines("flow",col.scale = tm_scale_intervals(
  n = 5,
  style = "fisher",
  values = "brewer.reds"),
  lwd = "flow",
   lwd.scale = tm_scale_continuous(values.range = c(0.01,0.8),
   values.scale = 9),
