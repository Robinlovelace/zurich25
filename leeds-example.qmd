...
library(cppRouting)
library(sfnetworks)
net_sfn = as_sfnetwork(network, directed = FALSE) |> 
  activate("nodes") |> 
...
net_dodgr_full = weight_streetnet(network, wt_profile = "motorcar")
...
# With just 1 OD pair:
graph_with_flows = dodgr_flows_aggregate(net_dodgr, from = od_for_dodgr$from, to = od_for_dodgr$to, flows = od_for_dodgr$flow, pairwise = TRUE)
# 5. Plot the results
...
```{r}
# routes_quiet = cyclestreets::batch(od_calculated, strategies = "quietest")
route_files_exist = file.exists("routes_quiet.gpkg")
if (!route_files_exist) {
  routes_quiet = stplanr::route(l = od_calculated, route_fun = cyclestreets::journey, plan = "quietest")
  sf::write_sf(routes_quiet, "routes_quiet.gpkg", delete_dsn = TRUE)
  system("gh release create v1")
  system("gh release upload v1 routes_quiet.gpkg --clobber")
}
rnet_quiet = stplanr::overline(routes_quiet, attrib = "interaction")
plot(rnet_quiet)
```